...
static int tryInsertAt(TableP table, const void *key, DataP object, size_t index, bool *inserted) {
    *inserted = false;
...
static int growTheTable(TableP table)
{
    ...
    for (size_t i = 0; i < table->tableSize * table->growthFactor; i++) {
        if (table->keyValues[i].key != NULL){
            newKeyValues[i*2].key = table->keyValues[i].key;
        }
    }
...
static int tryInsertAtHash(TableP table, const void *key, DataP object, int hash, bool* inserted)
{
    *inserted = false;
